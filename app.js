...
class MiStoreApp {
    handleLogin(event) {
        ...
        console.log('🔐 محاولة تسجيل الدخول:', { username, passwordLength: password.length });

        // التحقق من البيانات
        if (!username || !password) {
            loginMessage.textContent = '⚠️ يرجى إدخال اسم المستخدم وكلمة المرور';
    ...
    async previewImage(input) {
        ...
        if (input.files && input.files[0]) {
            ...
            // حفظ الصورة في المتغير مباشرة
            try {
                this.selectedProductImage = await this.getImageBase64(file);
                console.log('✅ تم حفظ الصورة في المتغير');
            } catch (error) {
    ...
    initData() {
        if (!this.storage.get('initialized')) {
            this.storage.set('products', [
                { id: 1, code: 'PH-001', name: 'Xiaomi Redmi Note 13', category: 'phones', purchasePrice: 180000, salePrice: 220000, quantity: 15, minQuantity: 5, description: 'هاتف شاومي ريدمي نوت 13', createdAt: new Date().toISOString() },
                { id: 2, code: 'PH-002', name: 'Xiaomi 14 Pro', category: 'phones', purchasePrice: 650000, salePrice: 780000, quantity: 8, minQuantity: 3, description: 'هاتف شاومي 14 برو', createdAt: new Date().toISOString() },
                { id: 3, code: 'AC-001', name: 'شاحن سريع 67W', category: 'chargers', purchasePrice: 15000, salePrice: 25000, quantity: 50, minQuantity: 10, description: 'شاحن سريع أصلي', createdAt: new Date().toISOString() },
                { id: 4, code: 'AC-002', name: 'كفر سيليكون شفاف', category: 'cases', purchasePrice: 3000, salePrice: 7000, quantity: 100, minQuantity: 20, description: 'كفر سيليكون', createdAt: new Date().toISOString() },
                { id: 5, code: 'AC-003', name: 'سماعات بلوتوث Redmi Buds', category: 'accessories', purchasePrice: 25000, salePrice: 40000, quantity: 30, minQuantity: 10, description: 'سماعات بلوتوث', createdAt: new Date().toISOString() },
                { id: 6, code: 'AC-004', name: 'واقي شاشة زجاجي', category: 'accessories', purchasePrice: 2000, salePrice: 5000, quantity: 3, minQuantity: 15, description: 'واقي شاشة', createdAt: new Date().toISOString() }
            ]);
            this.storage.set('suppliers', [
            ...
            this.storage.set('sales', [
                { id: 1, invoiceNumber: 'INV-2026-0001', date: new Date(Date.now() - 86400000).toISOString(), customerId: 1, customerName: 'أحمد محمد', items: [{ productId: 3, productName: 'شاحن سريع 67W', quantity: 2, price: 25000, total: 50000 }], subtotal: 50000, discount: 0, tax: 0, total: 50000, paid: 50000, remaining: 0, paymentMethod: 'cash', status: 'paid' }
            ]);
            this.storage.set('purchases', []);
            this.storage.set('stockMovements', []);
            this.storage.set('discounts', [
                { id: 1, code: 'WELCOME10', discount: 10, maxUses: 100, usedCount: 0, expiryDate: '2026-12-31', active: true, createdAt: new Date().toISOString() },
                { id: 2, code: 'SUMMER20', discount: 20, maxUses: 50, usedCount: 12, expiryDate: '2026-09-30', active: true, createdAt: new Date().toISOString() }
            ]);
            this.storage.set('offers', {
                title: 'عروض الصيف',
                startTime: new Date().toISOString(),
                endTime: new Date(Date.now() + 86400000).toISOString(),
                products: [
                    { productId: 3, discount: 15 },
                    { productId: 5, discount: 20 }
                ]
            });
            this.storage.set('chats', [
                {
                    id: 1,
                    customerName: 'أحمد محمد',
                    customerPhone: '07711112222',
                    unread: 2,
                    lastMessage: 'مرحباً، هل الكفر متوفر لهاتف Redmi Note 13؟',
                    messages: [
                        { from: 'customer', text: 'مرحباً، هل الكفر متوفر لهاتف Redmi Note 13؟', time: new Date(Date.now() - 3600000).toISOString() },
                        { from: 'customer', text: 'أريد معرفة السعر أيضاً', time: new Date(Date.now() - 1800000).toISOString() }
                    ],
                    createdAt: new Date(Date.now() - 3600000).toISOString()
                }
            ]);
            this.storage.set('settings', { storeName: 'Mi Store', storeAddress: 'بغداد - العراق', storePhone: '07700000000', currency: 'IQD', taxRate: 0 });
            // مستمع نموذج "من نحن"
    ...
    openProductModal(product = null) {
        ...
        // إعادة تعيين الصورة المختارة عند فتح النافذة
    ...
    deleteProduct(id) {
        this.modal.confirm('حذف المنتج', 'هل أنت متأكد؟', () => {
            let products = this.storage.get('products', []);
    ...
    saveStockMovement(type) {
        ...
        const movements = this.storage.get('stockMovements', []);
        movements.push({
        ...
        this.storage.set('stockMovements', movements);
    ...
    saveSale() {
        ...
        // ===== ربط المبيعات بالمخزون - خصم الكمية =====
        const products = this.storage.get('products', []);
        ...
        if (customerId && sale.remaining > 0) {
            ...
            if (customer) {
                ...
                this.storage.set('customers', customers);
            }
        }
    ...
    printSale(id) {
        ...
        printWindow.document.write(`<!DOCTYPE html><html lang="ar" dir="rtl"><head><meta charset="UTF-8"><title>فاتورة ${sale.invoiceNumber}</title>
            <style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Segoe UI',Tahoma,Arial;padding:20px;color:#212529}
    ...
    cancelSale(id) {
        this.modal.confirm('إلغاء الفاتورة', 'سيتم إعادة الكميات إلى المخزون. هل أنت متأكد؟', () => {
            const sales = this.storage.get('sales', []);
            const sale = sales.find(s => s.id === id);
            if (!sale) return;
            const products = this.storage.get('products', []);
            ...
            this.storage.set('sales', sales);
            this.toast.success('تم', 'تم إلغاء الفاتورة وإرجاع الكميات');
            this.renderSalesTable();
            this.renderInventoryTable();
            this.renderProductsTable();
            this.updateStats();
    ...
    renderPurchasesTable() {
        const purchases = this.storage.get('purchases', []);
        ...
        const suppliers = this.storage.get('suppliers', []);
    ...
    savePurchase() {
        ...
        if (purchase.remaining > 0) {
            ...
            this.storage.set('suppliers', suppliers);
        }
    ...
    // ========== إضافة هذه الدوال إلى class MiStoreApp قبل القوس المغلق الأخير ==========
    ...
    deleteSupplier(id) {
        ...
        this.modal.confirm('حذف المورد', 'هل أنت متأكد؟', () => {
            let suppliers = this.storage.get('suppliers', []);
    ...
    viewSupplier(id) {
        ...
        const supplier = suppliers.find(s => s.id === id);
        if (!supplier) return;
        const purchases = this.storage.get('purchases', []).filter(p => p.supplierId === id);
    ...
    deleteCustomer(id) {
        ...
        this.modal.confirm('حذف العميل', 'هل أنت متأكد؟', () => {
            let customers = this.storage.get('customers', []);
    ...
    viewCustomer(id) {
        ...
        const customer = customers.find(c => c.id === id);
        if (!customer) return;
        const sales = this.storage.get('sales', []).filter(s => s.customerId === id);
    ...
    saveDiscount(id) {
        ...
        if (id) {
            ...
            if (index !== -1) {
                ...
                this.storage.set('discounts', discounts);
    ...
    deleteDiscount(id) {
        this.modal.confirm('حذف كود الخصم', 'هل أنت متأكد؟', () => {
            let discounts = this.storage.get('discounts', []);
    ...
    // ===== عروض اليوم =====
    loadOfferSettings() {
        const offers = this.storage.get('offers', { title: '', startTime: '', endTime: '', products: [] });
    ...
    saveOfferSettings() {
        ...
        this.storage.set('offers', offers);
    ...
    removeOfferProduct(productId) {
        this.modal.confirm('إزالة من العروض', 'هل أنت متأكد؟', () => {
            const offers = this.storage.get('offers', { products: [] });
    ...
    sendReply() {
        ...
        // إعادة فتح المحادثة لتحديث الواجهة
    ...
    generateSalesReport() {
        ...
        const sales = this.storage.get('sales', []).filter(s => s.status !== 'cancelled');
        const now = new Date();
    ...
    printSelectedInvoice() {
        ...
        if (type === 'sales') this.printSale(id);
        else this.printPurchase(id);
        this.modal.close();
    }
    ...
    // ===== أدوات مساعدة إضافية =====
    generateCode(prefix) {
        const items = this.storage.get(prefix === 'SUP' ? 'suppliers' : 'customers', []);
